...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            box_corners = tools.compute_box_corners(label.box.center_x, label.box.center_y, label.box.width, label.box.length, label.box.heading)
            
            
            ## step 2 : loop over all detected objects
            for det in detections:

                ## step 3 : extract the four corners of the current detection (use tools.compute_box_corners)
                _score, _x, _y, _z, _h, _w, _l, _yaw = det
               
                det_corners = tools.compute_box_corners(_x, _y, _w, _l, _yaw)
                
                ## step 4 : compute the center distance between label and detection bounding-box in x, y, and z
                distance_center = np.array([label.box.center_x, label.box.center_y, label.box.center_z]) - np.array([_x, _y, _z])
               
                
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                box_poly = Polygon(box_corners)
                det_poly = Polygon(det_corners)
                intersection=box_poly.intersection(det_poly).area
                union=box_poly.union(det_poly).area
                ...
                if iou > min_iou:
                   # dist_x, dist_y, dist_z = distance_center
                    dist_x, dist_y, dist_z = distance_center
